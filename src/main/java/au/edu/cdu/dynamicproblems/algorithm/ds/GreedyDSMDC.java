...
public class GreedyDSMDC implements ITask, IGreedyDS<Integer> {

	@SuppressWarnings("unused")
	private static Logger log = LogUtil.getLogger(GreedyDSMDC.class);
	...
	public GreedyDSMDC(List<String[]> adjacencyMatrix, int k, int rUpperBoundary) {
	...
	private void greedy()
		...
		/* apply poly-rr */
		Graph<Integer, String> gPoly = GreedyDSUtil.applyPolyReductionRules(gOriginal, this.runningTimeMap,
		...
		for (Set<Integer> component : components) {
			List<Integer> componentVList = new ArrayList<Integer>(component);
			Graph<Integer, String> g = new SparseMultigraph<Integer, String>();
			AlgorithmUtil.prepareGraph(gPoly, g, componentVList);

			List<Integer> tempDS=new ArrayList<Integer>();
			/* apply degree-rr */
...
//			Map<Integer, Boolean> gDominatedMap = GreedyDSUtil.presetDominatedMap(componentVList);
//			List<Integer> orderedVList=OrderPackageUtil.getVertexListUtilityDesc(g, gDominatedMap);
//			
//			List<Integer> dI=new ArrayList();
//			dI.add(orderedVList.get(0));
			...
			GreedyDSUtil.applyMinimal(g, tempDS, this.runningTimeMap);
			GreedyDSUtil.applyLS(g, tempDS, this.runningTimeMap);
			
			this.dominatingSet.addAll(tempDS);
		}
