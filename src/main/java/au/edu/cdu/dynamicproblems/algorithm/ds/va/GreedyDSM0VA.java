...
@Deprecated
public class GreedyDSM0VA implements ITask, IAlgorithm {

	@SuppressWarnings("unused")
	private static Logger log = LogUtil.getLogger(GreedyDSM0VA.class);
	...
	public GreedyDSM0VA(List<String[]> adjacencyMatrix) {
	...
	public GreedyDSM0VA(List<String[]> adjacencyMatrix, int k, int rUpperBoundary) {
		this.adjacencyMatrix = adjacencyMatrix;
		this.numOfVertices = adjacencyMatrix.size();
		this.g = AlgorithmUtil.prepareGraph(this.adjacencyMatrix);
		this.k = k;
	...
	public GreedyDSM0VA(String indicator, List<String[]> adjacencyMatrix) {
		...
		for (i = 1; i < this.numOfVertices; i++) {
			...
			if (!dominatedMapI.get(v)) {
				...
				if (AlgorithmUtil.isMomentOfRegret(v, g, dI, u)) {
					...
					DDSFPTVA ag = new DDSFPTVA(indicator, gICopy, dK, rUpperBoundary);
