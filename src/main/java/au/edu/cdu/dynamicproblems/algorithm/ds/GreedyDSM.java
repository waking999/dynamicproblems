...
public class GreedyDSM implements ITask, IAlgorithm {

	@SuppressWarnings("unused")
	private static Logger log = LogUtil.getLogger(GreedyDSM.class);
	...
	public GreedyDSM(List<String[]> adjacencyMatrix) {
	...
	public GreedyDSM(List<String[]> adjacencyMatrix, int k, int rUpperBoundary) {
		this.adjacencyMatrix = adjacencyMatrix;
		this.numOfVertices = adjacencyMatrix.size();
		this.g = AlgorithmUtil.prepareGraph(this.adjacencyMatrix);
		this.k = k;
	...
	public GreedyDSM(String indicator, List<String[]> adjacencyMatrix) {
	...
	private void greedy() throws MOutofNException, ExceedLongMaxException, ArraysNotSameLengthException {
		...
		for (i = 1; i < this.numOfVertices; i++) {
			...
			if (!dominatedMapI.get(v)) {
				 ...
				 */
				gI = AlgorithmUtil.copyGrapy(gIPre);
