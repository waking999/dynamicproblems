...
@Deprecated
public class GreedyDSVS02VA implements IGreedyDSVA, ITask {

	@SuppressWarnings("unused")
	private static Logger log = LogUtil.getLogger(GreedyDSVS2VATest.class);
	...
	public GreedyDSVS02VA(String indicator, List<String[]> am, int k, int r) {
	...
	private void start()
		...
		while (!AlgorithmUtil.isAllDominated(dominatedMap)) {
			...
			DDSFPTVA ag2 = useDDSFPTSubToCalcDS(gOriginalVerticeSize, kVerticesDS, kVertices, gI);
	...
	@SuppressWarnings("unused")
	private GreedyNativeV1VA useGreedyNativeCalcDS(Graph<Integer, Integer> gI) throws InterruptedException {

		GreedyNativeV1VA ag = new GreedyNativeV1VA(gI);
		ag.run();
		return ag;

	}

	private DDSFPTVA useDDSFPTSubToCalcDS(int gOriginalVerticeSize, List<Integer> kVerticesDS, List<Integer> kVertices,
