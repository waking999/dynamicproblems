 ...
 *
 */
public class GreedyDSM3 implements ITask, IGreedyDS<Integer> {

	@SuppressWarnings("unused")
	private static Logger log = LogUtil.getLogger(GreedyDSM3.class);
...
//	private int numOfVertices;
	...
	public GreedyDSM3(List<String[]> adjacencyMatrix, int k, int rUpperBoundary) {
	...
	private void greedy() throws MOutofNException, ExceedLongMaxException, ArraysNotSameLengthException, InterruptedException {
		...
		int currentPos = 0;
		 ...
		 do {
			...
			/*
			 * if v is not dominated, i) get its highest utility neighbor;ii)
			 * put it in DI; and iii) construct a GI with them; and iv)
			 * calculate the position of the queue to put DI and GI; v) if it is
			 * a moment of regret, apply dds fpt; vi) and put the smaller
			 * solution ( of DI and DDSI ) and GI in the queue; vii) move the
			 * cursor of previousPos of the queue to currentPos;viii) because DI
			 * changes, we check the dominatedMap, if all have been dominated,we
			 * will quit the loop
			 */
			...
			/*
			 * the subgraph in each round: add u and one of its neighbor (other
			 * than vertices in previous sub graph)
			 *
			 * the purpose of adding u and only one of its neighbor is: 1. if
			 * only u, then sometimes u can be removed by dds fpt in a subgraph
			 * that u is dominated by another vertex but this makes other
			 * vertices dominated by u will be undominated; 2. if add all
			 * neighbors of u, the distance between G and G' can be huge such
			 * that slow down the process of dds fpt; 3. only u and one of its
			 * neighbor is a balance to not lose the generality and 2k is
			 * acceptable distance;
			 */
			// iii)
