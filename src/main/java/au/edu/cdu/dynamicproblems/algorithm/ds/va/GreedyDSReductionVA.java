...
@Deprecated
public class GreedyDSReductionVA implements IGreedyDSVA, ITask {
	...
	private static Logger log = LogUtil.getLogger(GreedyDSReductionVA.class);
	...
	public GreedyDSReductionVA(String indicator, List<String[]> am, int k, int r, int strategy,
	...
	public GreedyDSReductionVA(String indicator, int strategy, boolean withReductionRule) {
		this.indicator = indicator;
	...
	private void start()
		...
		while (undomiantedVerticesSize > 0) {
			...
			DDSFPTVA ag = new DDSFPTVA(indicator, gInitialCopy, dsInitial, paramR);
