...
@Deprecated
public class GreedyDSVS13VA implements IGreedyDSVA, ITask {

	@SuppressWarnings("unused")
	private static Logger log = LogUtil.getLogger(GreedyDSVS13VA.class);
	...
	public GreedyDSVS13VA(String indicator, List<String[]> am, int k, int r) {
	...
	private void start()
		...
		while (!AlgorithmUtil.isAllDominated(dominatedMap)) {
			...
			DDSFPTVA ag2 = useDDSFPTSubToCalcDS(gOriginalVerticeSize, kVerticesDS, kVertices, gI, greedyDiffSize);
	...
	private List<Integer> useGreedyToCalcDS(Graph<Integer, Integer> gI) throws InterruptedException {
		...
		GreedyVoteVA ag1 = new GreedyVoteVA(gI);
		ag1.run();
