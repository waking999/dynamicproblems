...
public class GreedyDSV4 implements IGreedyDS, ITask {
	...
	public GreedyDSV4(String indicator, List<String[]> am, int k, int r, List<Integer> ds1) {
	...
	// private Integer getHighestUtilityNeighborOfAVertex(Integer v,
	...
	private void preprocess() {
		...
		// for (Integer u : vNegb) {
		//
		// addDominatingVertexAndItsNeigbors(this.dsInitial,
		// this.initialVertices, u);
		// }
		//
		// }
		// }
		...
		//
		// // get u,w's degree
		// int uUtility = AlgorithmUtil.getVertexUtility(gOriginal, u,
		// dominatedMap);
		// int wUtility = AlgorithmUtil.getVertexUtility(gOriginal, w,
		// dominatedMap);
		//
		...
		// // dominating
		// // set than w
		// addHigherNeighborOfVToDS(this.dsInitial, v, u, w, uNegb, wNegb,
		// initialVertices, uUtility,
		// wUtility);
		// } else {
		// addHigherNeighborOfVToDS(this.dsInitial, v, w, u, wNegb, uNegb,
		// initialVertices, wUtility,
		// uUtility);
		// }
		//
		// }
	...
	// private Collection<Integer> getClosedNeighborsWithoutV(Integer v, Integer
	// w) {
	...
	// }
	//
	// private void addHigherNeighborOfVToDS(List<Integer> ds, Integer v,
	// Integer u, Integer w, Collection<Integer> uNegb,
	// Collection<Integer> wNegb, List<Integer> initalVerteices, int uUtility,
	// int wUtility) {
	// if (AlgorithmUtil.isAllDominated(dominatedMap, wNegb) && (wUtility - 1)
	// == 0) {
	//
	// addNeighborOfVToDS(ds, v, u, uNegb, initalVerteices);
	// } else if (AlgorithmUtil.isAllDominated(dominatedMap, uNegb) && (uUtility
	// - 1) == 0) {
	...
	// private void addNeighborOfVToDS(List<Integer> ds, Integer v, Integer w,
	// Collection<Integer> wNegb,
	...
	private void start()
		...
		List<Integer> removeDS = new ArrayList<Integer>();
		removeDS.addAll(dsInitial);
		int removeDSSize = removeDS.size();

		int ds1Size = ds1.size();

		while (!AlgorithmUtil.isAllDominated(dominatedMap) && removeDSSize < ds1Size) {

			Tuple<List<Integer>, List<Integer>> t = getKVertices(ds1, removeDS, undominatedVertices, k, gOriginal,
					gInitial);

			Collection<Integer> considerableCandidateVertices4DS = t.getV2();
			removeDS.addAll(considerableCandidateVertices4DS);
			removeDSSize = removeDS.size();
			...
			dominatedUndominatedDifference.clear();
		...
		// TreeMap<Integer, Integer> vdMap =
		// AlgorithmUtil.sortVertexMapAccordingToUtilityInclude(gOriginal,
		// dominatedMap, undominatedVertices);
		// TreeMap<Integer, Integer> allVdMap =
		// AlgorithmUtil.sortVertexMapAccordingToUtility(gOriginal,
		// dominatedMap);
		//
		// List<Integer> vList = AlgorithmUtil.getVertexListFromMap(vdMap,
		// fromIndex, toIndex);
		//
		// for (Integer u : vList) {
	...
	private Tuple<List<Integer>, List<Integer>> getKVertices(List<Integer> ds1, List<Integer> ds2,
			Collection<Integer> candidateVertices, int k, Graph<Integer, Integer> gOriginal,
			Graph<Integer, Integer> gComputing) {
		List<Integer> ds1SubtractDs2 = (List<Integer>) CollectionUtils.subtract(ds1, ds2);
		int ds1SubtractDs2Size = ds1SubtractDs2.size();

		List<Integer> kVerticesList = new ArrayList<Integer>();
		List<Integer> dsKList = new ArrayList<Integer>();

		int index = 0;
		while (kVerticesList.size() < k && index < ds1SubtractDs2Size) {

			Integer v = ds1SubtractDs2.get(index);
			...
			for (Integer u : candidateNeg) {
				if (!gComputing.containsVertex(u)) {
