...
public class GreedyDSMVWWSTest {
	private Logger log = LogUtil.getLogger(GreedyDSMVWWSTest.class);
	private static final String CLASS_NAME = GreedyDSMVWWSTest.class.getSimpleName();
		...
		for (TestParameter tp : tps) {
			...
			if (tp.isBeTest()) {
				...
				for (int i = iStart; i <= iEnd; i++) {
					...
					IGreedyDS<Integer> ag01 = new GreedyDSM1WW(am, k, r);
