...
public class GreedyDSMVSV1Test {
	private Logger log = LogUtil.getLogger(GreedyDSMVSV1Test.class);
	private static final String CLASS_NAME = GreedyDSMVSV1Test.class.getSimpleName();
			...
			if (tp.isBeTest()) {
				...
				for (int i = 1; i <= 1; i++) {
					...
					for (int j = kLow; j < kUp; j++) {
						...
						IGreedyDS<Integer> ag01 = new GreedyDSM1V1(am, j, j);
		...
		for (TestParameter tp : tps) {
			...
			if (tp.isBeTest()) {
				...
				for (int i = iStart; i <= iEnd; i++) {
					...
					IGreedyDS<Integer> ag01 = new GreedyDSM1V1(am, k, r);
					Result result01 = ag01.run();
					List<Integer> ds01 = ag01.getDominatingSet();
					int ds01Size = ds01.size();
					Map<String, Long> ag01RunningTimeMap = ag01.getRunningTimeMap();
