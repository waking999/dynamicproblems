...
@Deprecated
public class GreedyIterativeVATest {

	private Logger log = LogUtil.getLogger(GreedyIterativeVATest.class);
			...
			for (String file : files) {
				...
				for (int k = iStart; k <= iEnd; k++) {
					...
					for (int i = 0; i < times[j]; i++) {
						...
						GreedyIterativeV0VA ag = new GreedyIterativeV0VA(this.getClass().getName(),am);
						ag.run();
