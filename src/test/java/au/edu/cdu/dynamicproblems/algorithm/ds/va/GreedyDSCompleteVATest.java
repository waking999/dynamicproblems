...
@Deprecated
public class GreedyDSCompleteVATest {
	Logger log = LogUtil.getLogger(GreedyDSCompleteVATest.class);
	...
	private void run(String inputFile, int[][] krArray, String destFile)
		...
		for (int[] kr : krArray) {
			...
			for (int r = 1; r <= rUpper; r++) {
				...
				GreedyDSCompleteVA ag = new GreedyDSCompleteVA(this.getClass().getName(), am, k, r);
