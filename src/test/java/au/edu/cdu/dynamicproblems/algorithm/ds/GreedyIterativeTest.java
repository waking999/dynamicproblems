...
public class GreedyIterativeTest {
	...
	public void testKONECT_verify() throws InterruptedException, IOException, FileNotFoundException {
		...
		run(path, TestUtil.KONECT_TP, destFile, times, 1, 1);
	...
	public void testDIMACS_verify() throws InterruptedException, IOException, FileNotFoundException {
		...
		run(path, TestUtil.DIMACS_TP, destFile, times, 1, 1);
	}
	...
	public void testBHOSLIB_verify() throws InterruptedException, IOException, FileNotFoundException {
		...
		run(path, TestUtil.BHOSLIB_TP, destFile, times, 1, 1);
	}

	private void run(String path, TestParameter[] tps, String destFile, int[] times, int iStart, int iEnd)
		...
		for (int j = 0; j < times.length; j++) {
			...
			for (TestParameter tp : tps) {
				...
				for (int k = iStart; k <= iEnd; k++) {
					...
					roundResultArr[k - iStart] = new RoundResult(finalSize, finalRunningtime,tp.getFile(),k);
